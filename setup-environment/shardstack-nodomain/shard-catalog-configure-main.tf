...
resource "null_resource" "sdb_shard_catalog_configure" {
  ...
  provisioner "file" {
    content     = <<-EOF
      ...
      spool off
      alter system set local_listener='${oci_database_db_system.catalog_db[count.index].hostname}:${oci_database_db_system.catalog_db[count.index].listener_port}' scope=both;
