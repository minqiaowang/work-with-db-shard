...
resource "null_resource" "sdb_add_shard" {
  ...
  provisioner "file" {
    content     = <<-EOF
      ...
      gdsctl add cdb -connect ${data.oci_core_vnic.shard_db_node_vnic[count.index].public_ip_address}:${oci_database_db_system.shard_db[count.index].listener_port}/${data.oci_database_database.shard_database[count.index].db_unique_name} -pwd sd${random_string.gsmrootuser_pass.result}
